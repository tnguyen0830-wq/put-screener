...
reached the API, #130) from `bad-key`, `quota`, `bad-voice` and
`unavailable`, and a 200 whose body is JSON or HTML rather than audio is a
failure, not a finished read — playing an empty file is silence that reads
as a mute device.
