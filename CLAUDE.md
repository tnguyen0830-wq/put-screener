...
**THREE UW endpoints have no per-strike gamma — measured 2026-09-06. The sentence that used to sit here said "nothing UW sells on this plan", and that was an overstatement retracted in #208.** The probe's first production run measured exactly three stock endpoints: `greek-exposure` is 251 rows keyed by **`date`** (a daily time series of chain-wide gamma/delta/charm/vanna, no strikes at all), `spot-exposures` is 564 rows keyed by **`time`/`start_time`** with a `price` column, and `gex-levels` is the four aggregate levels already in use. All three findings still stand.

What does **not** follow is a claim about UW's whole catalogue. The owner reads UW's own site, which renders a per-strike gamma/delta tab in real time — a **second surface**, and it says the backend has the data. That is the #108 shape exactly: infer from one surface, then a second surface overturns it, which is the very lesson written at the top of the SPX section and which this file then repeated. The open question is narrow and unmeasured: **does the API on this plan expose a per-strike endpoint.**

If it does, the place it would genuinely win is **SPX and nowhere else**. SPY/QQQ/IWM already come from the live Schwab chain; SPX falls to CBOE at a **15-minute delay** because of the #108 defect, so UW real-time would be a real improvement there. The three reasons not to reach for UW casually (the 30,000/day cap #78 burned through, two computation paths that drift #96/#99/#147, and UW's `vol` basis versus this app's `oi`) are design constraints on *how* to use it — they were never reasons to claim it lacked the data.
