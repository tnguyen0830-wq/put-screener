...
No PR is currently open and unmerged as of #159. If you're reading this and a
...
The other three: how much of the **monthly cap** is left (X bills by *posts
read per month*, not requests, so that number decides cheap versus expensive);
whether **`since_id` is honoured**, which is the mechanism that keeps the cost
down (a re-poll with no new posts must return zero, or the monthly cap
evaporates re-reading the same posts); and the **query-length ceiling**, probed
...
monthly cap, and a test pins that the response never contains the token.
