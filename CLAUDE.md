...
2026-09-06 — Waiting on one more production reading of `/api/uwprobe?symbol=SPX`: the first one answered the per-strike question (no) but the probe's own heuristic misfired, and the follow-up measures whether `spot-exposures` is a gamma-vs-price curve or just a time series. Branch: claude/uwprobe-fix-heuristic. **SPX is settled:** all three spellings measured on production return gamma=0 and openInterest=0 on all 3600 contracts - the account has no index-option market data, so no code change can compute SPX GEX. UW is the source for SPX and the screen says so. See the GEX section for the full reading and for the one untried idea (SPY chain ×10 as an SPX proxy).
...
No PR is currently open and unmerged as of #106. If you're reading this and a
