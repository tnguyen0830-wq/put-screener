...
There is no test runner configured. **Network from this sandbox is an allowlist, not a blackout — and the owner can change it.** An earlier version of this file said "no outbound network", which is wrong in a way that matters. Measured 2026-09-16: `api.github.com` and `registry.npmjs.org` answer **200**, while every market-data host this app uses is refused by the egress proxy with a **403 on CONNECT** — `api.schwabapi.com`, `cdn.cboe.com`, `data.sec.gov`, `api.unusualwhales.com`, `api.tastyworks.com`, `unitedstates.github.io`, `finviz.com`, `financialmodelingprep.com`, `api.telegram.org`. **Check rather than assume**: `curl -sS "$HTTPS_PROXY/__agentproxy/status"` prints the proxy state and its most recent refusals by host, and a plain `curl -o /dev/null -w "%{http_code}"` settles any one host in a second.

**The practical rule is unchanged** — anything touching a live API can still only be verified in production, which is why the self-diagnosing idiom below matters so much, and why real numbers from the owner's Schwab app have repeatedly been the thing that caught bugs the tests missed. What changes is that *"impossible"* was never the right word. Three of those hosts need **no key and expose nothing**: `cdn.cboe.com`, `data.sec.gov`/`www.sec.gov`, `unitedstates.github.io`. Adding them to the environment's network settings would let a session verify the CBOE chain shape (#109, still a guess), a real Form 4, and the portrait URLs (#133) *directly* rather than shipping a guess with a self-diagnosing error attached. The **keyed** hosts (Schwab, UW, tastytrade) are a different call: opening the host is harmless, but *calling* one needs credentials, and credentials pasted into a session live in that session's transcript — so those stay measured by the production probes (`/api/uwprobe`, `/api/ttprobe`) instead.
...
No PR is currently open and unmerged as of #141. If you're reading this and a
