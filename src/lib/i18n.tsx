...
const DICT: Record<string, Record<Lang, Entry>> = {
  ...
  'lf.introTrades': {
    vi: 'TỪNG LỆNH KHỚP quyền chọn toàn thị trường, qua luồng WebSocket của Unusual Whales — giống màn Live Flow của UW. Phía ASK/BID/MID là giá khớp so với NBBO lúc khớp. Luồng này CHƯA được đo trên tài khoản của bạn (có thể là gói riêng) — nếu không chạy, phần chẩn đoán bên dưới in đúng lời UW trả về.',
    en: 'EVERY options fill across the whole market, via the Unusual Whales WebSocket feed — like UW’s own Live Flow screen. ASK/BID/MID is the fill against the NBBO at that moment. This feed is UNMEASURED on your account (it may be a separate plan) — if it does not run, the diagnostics below print exactly what UW sent back.',
  },
  ...
  'lf.ws.http': {
    vi: (v: any) => `Hỏi thẳng địa chỉ socket bằng HTTP: UW trả ${v.status} — ${v.meaning}. Nguyên văn:`,
    en: (v: any) => `Asking the socket address over plain HTTP: UW answered ${v.status} — ${v.meaning}. Verbatim:`,
  },
  'lf.ws.httpErr': {
    vi: (e: string) => `Hỏi thẳng địa chỉ socket bằng HTTP cũng không tới được: ${e}`,
    en: (e: string) => `Asking the socket address over plain HTTP also failed to connect: ${e}`,
  },
  'lf.ws.h401': { vi: 'khoá API bị từ chối', en: 'the API key was rejected' },
  ...
  'lf.ws.h426': { vi: 'địa chỉ có thật và chỉ nhận WebSocket — tức lỗi nằm ở bước bắt tay, không phải ở quyền', en: 'the address exists and only accepts WebSocket — so the failure is in the handshake, not in permissions' },
