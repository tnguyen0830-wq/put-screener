 ...
 *
 * Chi phí: đúng HAI request (flow-alerts lọc `ticker_symbol` — tham số đã
...
import type { UwContext } from './uwsummary';
...
export async function loadUwContext(symbol: string, now = Date.now()): Promise<UwContext> {
  ...
  try {
    const payload = await uwGet('/api/option-trades/flow-alerts', { ticker_symbol: t, limit: 200 });
    const raws = rowsOf(payload);
    const rows: LiveRow[] = [];
    for (const r of raws) {
      const row = parseLiveRow(r);
      if (row) rows.push(row);
      else base.flow.unparsed++;
    }
    ...
    if (raws.length && !rows.length) base.flow.sampleKeys = Object.keys(raws[0] ?? {}).slice(0, 40);
  } catch (e) {
